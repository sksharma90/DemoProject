...
public class MultipleSubject extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void VerifyMultipleSubject() throws Exception {

		// logger = report.createTest("Verify the Functionality By Selecting Multiple
		// Subjects");
		...
		ExtentFactory.getInstance().getExtent().info("Selecting Multiple Subject");
		Thread.sleep(400);
		...
		{
			constants.fileone.setCellData(sheetName, "Status", 238, "Pass");
			Thread.sleep(300);
			From.EducationalQualificationCancel().click();
		}

		else {
			constants.fileone.setCellData(sheetName, "Status", 238, "Fail");
		}
		...
		ExtentFactory.getInstance().getExtent()
				.info("Successfully Applied Educational Details By Selecting Multiple PG Graduation Subject scenario");
	}
