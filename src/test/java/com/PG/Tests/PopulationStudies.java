...
public class PopulationStudies extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void PopulationStudiesWithAtleast45percentage() throws Exception {

		// logger = report.createTest("Verifying PopulationStudies PG Department Apply
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting PopulationStudies");
			Thread.sleep(500);
	...
	@Test(priority = 2)
	public void PopulationStudiesWithLessThan45percentage() throws Exception {

		// logger = report.createTest("Verifying PopulationStudies Department Not Apply
		// Less Than 45 Percenatge");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 206, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 206, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
