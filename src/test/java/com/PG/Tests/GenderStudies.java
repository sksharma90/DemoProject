...
public class GenderStudies extends BaseClass implements PGCAFData{
	...
	@Test(priority = 1)
	public void GenderStudiesWithAtleast45Percentage() throws Exception {

		//logger = report.createTest("Verify Gender Studies Apply Atleast 45 Percentage");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting GenderStudies");
			Thread.sleep(400);
			From.PGSubjectofAdmission("Gender Studies");
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 127, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationCancel().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 127, "Fail");
			}
			...
			ExtentFactory.getInstance().getExtent().info("Successfully Checked GenderStudies PG Department Atleast 45 Percentage scenario");
		}

	}

	@Test(priority = 2)
	public void GenderStudiesWithLessThan45Percentage() throws Exception {

		//logger = report.createTest("Verify Gender Studies with Less Than 45 Percentage");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 128, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 128, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked GenderStudies PG Department with Less Than 45 Percentage scenario");
		}
