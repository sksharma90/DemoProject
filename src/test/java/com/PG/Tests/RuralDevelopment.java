...
public class RuralDevelopment extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void RuralDevelopmentWithAtleast45Percentage() throws Exception {
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting Rural Development");
			Thread.sleep(600);
			From.PGSubjectofAdmission("Rural Development");
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 213, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationCancel().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 213, "Fail");
			}
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked Rural Development Atleast 45 Percentage scenario");
		}

	}

	@Test(priority = 2)
	public void RuralDevelopmentWithLessThan45Percentage() throws Exception {
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 214, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 214, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked Rural Development with Less Than 45 Percentage scenario");
		}
