...
public class Philosophy extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void PhilosophyWithAtleast45percentage() throws Exception {

		// logger = report.createTest("Verifying Philosophy PG Department Apply Minimum
		// 45 Percentage");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting Philosophy");
			Thread.sleep(600);
			From.PGSubjectofAdmission("Philosophy");

			Thread.sleep(400);

			// From.CollegeorUniversityPreference("Khallikote (Autonomous) College,
			// Berhampur");
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 197, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationCancel().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 197, "Fail");
			}
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked Philosophy Department Apply Minimum 45 percentage Scenario");
		}

	}

	@Test(priority = 2)
	public void PhilosophyWithLessThan45percentage() throws Exception {

		// logger = report.createTest("Verifying Philosophy Department Not Apply Less
		// Than 45 Percenatge");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 198, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 198, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked Philosophy Department Not Apply Less Than 45 Percenatge Scenario");
		}
