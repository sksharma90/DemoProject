...
public class AppliedPsychology extends BaseClass implements PGCAFData{
	...
	@Test(priority = 1)
	public void AppliedPsychologyWithAtleast45Percentage() throws Exception {

		//logger = report.createTest("Verify Applied Psychology with Atleast 45 Percentage");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting Applied Psychology");
			Thread.sleep(400);
			From.PGSubjectofAdmission("Applied Psychology");
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 88, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationCancel().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 88, "Fail");
			}
			...
			ExtentFactory.getInstance().getExtent().info("Successfully Checked Applied Psychology PG Department with Atleast 45 Percentage");
		}

	}

	@Test(priority = 2)
	public void AppliedPsychologyWithLessThan45Percentage() throws Exception {

		//logger = report.createTest("Verify Applied Psychology with Less Than 45 Percentage");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 89, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 89, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked Applied Psychology PG Department with Less Than 45 Percentage");
		}
