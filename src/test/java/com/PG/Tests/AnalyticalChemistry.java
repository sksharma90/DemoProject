...
public class AnalyticalChemistry extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void AnalyticalChemistryWithAtleast45percentage() throws Exception {

		// logger = report.createTest("Verifying Analytical Chemistry PG Department
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting Analytical Chemistry");
			Thread.sleep(600);
			From.PGSubjectofAdmission("Analytical Chemistry");
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 68, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationCancel().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 68, "Fail");
			}
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked Analytical Chemistry Department Apply Minimum 45 percentage Scenario");
		}

	}

	@Test(priority = 2)
	public void AnalyticalChemistryWithLessThan45percentage() throws Exception {

		// logger = report.createTest("Verifying Analytical Chemistry Department Not
		// Apply Less Than 45 Percenatge");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 69, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 69, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
			...
			ExtentFactory.getInstance().getExtent().info(
					"Successfully Checked Analytical Chemistry Department Not Apply Less Than 45 Percenatge Scenario");
		}
	}
}
