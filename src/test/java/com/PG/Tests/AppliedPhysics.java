...
public class AppliedPhysics extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void AppliedPhysicsWithAtleast45percentage() throws Exception {

		// logger = report.createTest("Verifying Applied Physics PG Department with
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting Applied Physics");
			Thread.sleep(400);
			From.PGSubjectofAdmission("Applied Physics");
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 84, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationCancel().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 84, "Fail");
			}
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked Applied Physics Department with Minimum 45 percentage Scenario");
		}

	}

	@Test(priority = 2)
	public void AppliedPhysicsWithLessThan45percentage() throws Exception {

		// logger = report.createTest("Verifying Applied Physics Department with Less
		// Than 45 Percenatge");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 85, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 85, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked Applied Physics Department withLess Than 45 Percenatge Scenario");
		}
	}
}
