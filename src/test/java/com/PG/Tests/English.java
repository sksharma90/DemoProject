...
public class English extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void EnglishWithAtleast45percentage() throws Exception {

		// logger = report.createTest("Verifying for English PG department with Minimum
		// 45 Percentage");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("English");

			Thread.sleep(500);
			From.PGSubjectofAdmission("English");
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 121, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationCancel().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 121, "Fail");
			}
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully checked applying for English department with Minimum 45 percentage Scenario");
		}

	}

	@Test(priority = 2)
	public void EnglishWithLessThan45percentage() throws Exception {

		// logger = report.createTest("Verifying for English PG department with Less
		// Than 45 Percenatge");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 122, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 122, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully checked applying for English department with Less Than 45 Percenatge Scenario");
		}
