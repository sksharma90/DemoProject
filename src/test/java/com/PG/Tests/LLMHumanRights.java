...
public class LLMHumanRights extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void LLMHumanRightsWithAtleast50Percentage() throws Exception {

		// logger = report.createTest("Verify LLM Human Rights Apply Atleast 50
		// Percentage");
		...
		String[] arrData = { "Law(LLB)", "Integrated Law" };
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			if (defaultItem.equals("Law(LLB)")) {
				From.CoreorHons("Law");
			} else if (defaultItem.equals("Integrated Law")) {
				From.CoreorHons("Law");
			}

			ExtentFactory.getInstance().getExtent().info("Selecting LLM HumanRights");
			Thread.sleep(400);
			From.PGSubjectofAdmission("LLM (Human Rights)");
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked LLMHumanRights PG Department Atleast 50 Percentage");
		}

	}

	@Test(priority = 2)
	public void LLMHumanRightsWithLessThan50Percentage() throws Exception {

		// logger = report.createTest("Verify LLM Human Rights with Less Than 50
		// Percentage");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting LLMHumanRights");
			Thread.sleep(400);
			From.PGSubjectofAdmission("LLM (Human Rights)");
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 154, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 154, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
			...
			ExtentFactory.getInstance().getExtent()
					.info("Sucessfully Checked LLMHumanRights PG Department with Less Than 50 Percentage");
		}
