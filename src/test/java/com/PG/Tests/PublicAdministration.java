...
public class PublicAdministration extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void PublicAdministrationWithAtleast45percentage() throws Exception {

		// logger = report.createTest("Verifying Public Administration PG Department
		// Apply Minimum 45 Percentage");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Public Administration");
			Thread.sleep(400);
			From.PGSubjectofAdmission("Public Administration");
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 209, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationCancel().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 209, "Fail");
			}
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked Public Administration Department Apply Minimum 45 percentage Scenario");
		}

	}

	@Test(priority = 2)
	public void PublicAdministrationWithLessThan45percentage() throws Exception {

		// logger = report.createTest("Verifying Public Administration Department Not
		// Apply Less Than 45 Percentage");
		PGCAFNew From = new PGCAFNew(driver);
		Thread.sleep(400);
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting Public Administration");
			Thread.sleep(400);
			From.PGSubjectofAdmission("Public Administration");
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 210, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 210, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
			...
			ExtentFactory.getInstance().getExtent().info(
					"Successfully Checked Public Administration Department Not Apply Less Than 45 Percenatge Scenario");
		}
