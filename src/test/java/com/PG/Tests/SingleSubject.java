...
public class SingleSubject extends BaseClass implements PGCAFData {
	;
	XSSFWorkbook wb;
	XSSFSheet sht;
	...
	@Test(priority = 1)
	public void VerifySingleSubject() throws Exception {

		// logger = report.createTest("Verify the Functionality By Selecting Single
		// Subjects");
		...
		ExtentFactory.getInstance().getExtent().info("Selecting Single Subject");
		Thread.sleep(400);
		...
		{
			constants.fileone.setCellData(sheetName, "Status", 237, "Pass");
			Thread.sleep(300);
			From.EducationalQualificationCancel().click();
		}

		else {
			constants.fileone.setCellData(sheetName, "Status", 237, "Fail");
		}
		...
		ExtentFactory.getInstance().getExtent()
				.info("Successfully Applied Educational Details By Selecting Single PG Graduation Subject scenario");
	}
