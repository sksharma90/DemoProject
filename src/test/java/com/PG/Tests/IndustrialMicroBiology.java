...
public class IndustrialMicroBiology extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void IndustrialMicroBiologyDepartmentWithAtleast45percentage() throws Exception {

		// logger = report.createTest("Verifying Industrial Microbiology PG Department
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			if (defaultItem.equals("BTech")) {
				Thread.sleep(400);
				From.CoreorHons("Microbiology");
			}

			else if (defaultItem.equals("Science")) {
				Thread.sleep(300);
				String[] arrData1 = { "Botany", "Microbiology", "Zoology" };
				...
				for (int j = 0; j < arrData1.length; j++) {
					...
					ExtentFactory.getInstance().getExtent().info("Selecting Industrial Microbiology");
					Thread.sleep(400);
					From.PGSubjectofAdmission("Industrial Microbiology");
			...
			if (defaultItem.equals("BTech")) {
				...
				ExtentFactory.getInstance().getExtent().info(
						"Successfully Checked IndustrialMicroBiologyDepartment Apply Minimum 45 percentage Scenario");
			}
		}
	}

	@Test(priority = 2)
	public void IndustrialMicroBiologyDepartmentWithLessThan45percentage() throws Exception {

		// logger = report.createTest("Verifying IndustrialMicrobiology Department Not
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			if (defaultItem.equals("BTech")) {
				...
				ExtentFactory.getInstance().getExtent().info(
						"Successfully Checked IndustrialMicrobiology Department Not Apply Less Than 45 Percenatge Scenario");
			}
		}
	}
}
