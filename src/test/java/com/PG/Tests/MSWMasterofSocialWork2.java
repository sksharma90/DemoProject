...
public class MSWMasterofSocialWork2 extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void MSWWithAtleast45Percentage() throws Exception {

		// logger = report.createTest("Verify MSW Apply Atleast 45 Percentage");
		...
		String[] arrData = { "Science", "BSW", "Arts" };
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			if (defaultItem.equals("BSW")) {
				...
				ExtentFactory.getInstance().getExtent().info("Selecting MSW (Master of Social Work)");

				Thread.sleep(400);
				From.PGSubjectofAdmission("MSW (Master of Social Work)");

				Thread.sleep(400);

				// Thread.sleep(500);
				//From.CollegeorUniversityPreference("Udaya Nath College of Science & Technology,Adaspur");
				...
				From.CoreorHons("Anthropology(Science)");
				Thread.sleep(300);
				...
				Thread.sleep(300);
				String[] arrData1 = { "Political Science", "Psychology", "Sociology", "Anthropology(Arts)",
						"Economics" };
			...
			Thread.sleep(400);
			if (defaultItem.equals("Science") || (defaultItem.equals("BSW"))) {
				...
				ExtentFactory.getInstance().getExtent()
						.info("Successfully Checked MSW PG Department Atleast 45 Percentage scenario");
			}
		}
	...
	@Test(priority = 2)
	public void MSWWithLessThan45Percentage() throws Exception {

		// logger = report.createTest("Verify MSW Not Apply Less Than 45 Percentage");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			if (defaultItem.equals("Science") || (defaultItem.equals("BSW"))) {
				...
				{
					constants.fileone.setCellData(sheetName, "Status", 192, "Pass");
					Thread.sleep(300);
					From.EducationalQualificationOk().click();
				}

				else {
					constants.fileone.setCellData(sheetName, "Status", 192, "Fail");
				}
				SoftAssert softAssert = new SoftAssert();
				...
				ExtentFactory.getInstance().getExtent()
						.info("Successfully Checked MSW PG Department Not Apply Less Than 45 Percentage scenario");
			}
		}
