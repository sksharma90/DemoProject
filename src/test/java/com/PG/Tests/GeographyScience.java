...
public class GeographyScience extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void GeographyWithAtleast45percentage() throws Exception {

		// logger = report.createTest("Verifying Geography PG Department Apply Minimum
		// 45 Percentage");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			Thread.sleep(300);
			From.GraduationElective1("Economics");
			...
			ExtentFactory.getInstance().getExtent().info("Geography");
			Thread.sleep(600);
			From.PGSubjectofAdmission("Geography");
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 129, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationCancel().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 129, "Fail");
			}
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully checked applying for Economics department with Minimum 45 percentage Scenario");
		}

	}

	@Test(priority = 2)
	public void GeographyWithLessThan45percentage() throws Exception {

		// logger = report.createTest("Verifying Geography Department Not Apply Less
		// Than 45 Percenatge");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 130, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 130, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
			...
			ExtentFactory.getInstance().getExtent().info(
					"Successfully checked applying for Economics department with Less Than 45 Percenatge Scenario");
		}
