...
public class Stastics extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void StasticsWithAtleast45percentage() throws Exception {

		// logger = report.createTest("Verifying Stastics PG Department Apply Minimum 45
		// Percentage");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Statistics");
			Thread.sleep(1000);
			From.PGSubjectofAdmission("Statistics");

			Thread.sleep(400);

			From.StasticsYes().click();

			// From.CollegeorUniversityPreference("Ravenshaw University, Cuttack");

			Thread.sleep(400);
			From.FirstPreference("Ravenshaw University, Cuttack");
			Thread.sleep(400);
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 221, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationCancel().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 221, "Fail");
			}
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked Statstics Department Apply Minimum 45 percentage Scenario");
		}

	}

	@Test(priority = 2)
	public void StasticsWithLessThan45percentage() throws Exception {

		// logger = report.createTest("Verifying Statstics Department Not Apply Less
		// Than 45 Percenatge");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting Statistics");
			Thread.sleep(1000);
			From.PGSubjectofAdmission("Statistics");
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 222, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 222, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked Statstics Department Not Apply Less Than 45 Percenatge Scenario");
		}
	...
	public void NotEligibleCriteria() throws Exception {
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 223, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 223, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
