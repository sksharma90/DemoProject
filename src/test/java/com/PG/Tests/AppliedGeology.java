...
public class AppliedGeology extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void AppliedGeologyWithAtleast45percentage() throws Exception {

		// logger = report.createTest("Verifying Applied Geology PG Department with
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting Applied Geology");
			Thread.sleep(400);
			From.PGSubjectofAdmission("Applied Geology");
			...
			// Thread.sleep(500);
			// From.CollegeorUniversityPreference("Dharanidhar (Autonomous) College,
			// Keonjhar");
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 80, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationCancel().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 80, "Fail");
			}
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked Applied Geology Department with Minimum 45 percentage Scenario");
		}

	}

	@Test(priority = 2)
	public void AppliedGeologyWithLessThan45percentage() throws Exception {

		// logger = report.createTest("Verifying Applied Geology Department with Less
		// Than 45 Percenatge");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 81, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 81, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked Applied Geology Department with Less Than 45 Percenatge Scenario");
		}
	}
}
