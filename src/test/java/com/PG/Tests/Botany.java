...
public class Botany extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void BotanyWithAtleast45percentage() throws Exception {

		// logger = report.createTest("Verifying Botany PG Department Apply Minimum 45
		// Percentage");
		...
		String[] arrData = { "Botany", "Plant Science" };
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			From.GraduationElective1("Zoology");
			...
			ExtentFactory.getInstance().getExtent().info("Selecting Botany");
			Thread.sleep(1000);
			From.PGSubjectofAdmission("Botany");
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 101, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationCancel().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 101, "Fail");
			}
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked Botany Department with Minimum 45 percentage Scenario");
		}

	}

	@Test(priority = 2)
	public void BotanyWithLessThan45percentage() throws Exception {

		// logger = report.createTest("Verifying Botany Department Not Apply Less Than
		// 45 Percentage");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 102, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 102, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked Botany Department with Less Than 45 Percenatge Scenario");
		}
