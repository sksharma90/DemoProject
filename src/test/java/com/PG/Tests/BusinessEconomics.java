...
public class BusinessEconomics extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void BusinessEconomicsWithAtleast50Percentage() throws Exception {

		// logger = report.createTest("Verify Business Economics with Atleast 50
		// Percentage");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting Business Economics");
			Thread.sleep(400);
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 103, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationCancel().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 103, "Fail");
			}
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked BusinessEconomics PG Department Atleast 50 Percentage");
		}

	}

	@Test(priority = 2)
	public void BusinessEconomicsWithLessThan50Percentage() throws Exception {

		// logger = report.createTest("Verify Business Economics with Less Than 50
		// Percentage");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 104, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 104, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked BusinessEconomics PG Department with Less Than 50 Percentage");
		}
