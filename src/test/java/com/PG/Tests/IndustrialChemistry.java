...
public class IndustrialChemistry extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void IndustrialChemistryDepartmentWithAtleast45percentage() throws Exception {

		// logger = report.createTest("Verifying Industrial Chemistry PG Department
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting Industrial Chemistry");
			Thread.sleep(400);
			From.PGSubjectofAdmission("Industrial Chemistry");
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked IndustrialChemestryDepartment Apply Minimum 45 percentage Scenario");
		}

	}

	@Test(priority = 2)
	public void IndustrialChemistryDepartmentWithLessThan45percentage() throws Exception {

		// logger = report.createTest("Verifying Industrial Chemistry Department Not
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info(
					"Successfully Checked IndustrialChemestry Department Not Apply Less Than 45 Percenatge Scenario");
		}
	}
}
