...
public class MasterInPublicHealth extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void MPHWithAtleast45Percentage() throws Exception {
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting MPH (Master in Public Health)");
			Thread.sleep(400);
			From.PGSubjectofAdmission("MPH (Master in Public Health)");
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 185, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationCancel().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 185, "Fail");
			}
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked MPH (Master in Public Health) Atleast 45 Percentage scenario");
		}

	}

	@Test(priority = 2)
	public void MPHWithLessThan45Percentage() throws Exception {
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 186, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 186, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked MPH (Master in Public Health) with Less Than 45 Percentage scenario");
		}
