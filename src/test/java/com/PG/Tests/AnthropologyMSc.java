...
public class AnthropologyMSc extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void AnthropologyMScWithAtleast45percentage() throws Exception {

		// logger = report.createTest("Anthropology MSC PG Department with Minimum 45
		// Percentage");
		...
		String[] arrData = { "Zoology", "Microbiology", "Geology", "Biotechnology", "Botany", "Anthropology(Science)" };
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting Anthropology (M.Sc)");
			Thread.sleep(1000);
			From.PGSubjectofAdmission("Anthropology(Science)");
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 74, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationCancel().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 74, "Fail");
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked Anthropology MSC Department with Minimum 45 percentage Scenario");
		}

	}

	@Test(priority = 2)
	public void AnthropologyMScWithLessThan45percentage() throws Exception {

		// logger = report.createTest("Anthropology MSC Department with Less Than 45
		// Percenatge");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 75, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 75, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked Anthropology MSC Department with Less Than 45 Percenatge Scenario");
		}
