...
public class AIHCADepartment extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void AIHCAWithAtleast45percentage() throws Exception {

		// logger = report.createTest("Verifying AIHCA PG Department Apply Minimum 45
		// Percentage");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting AIHCA (Ancient Indian History Culture and Archeology )");
			Thread.sleep(400);
			From.PGSubjectofAdmission("AIHCA (Ancient Indian History Culture and Archeology )");
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 64, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationCancel().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 64, "Fail");
			}
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked AIHCA Department Apply Minimum 45 percentage Scenario");
		}

	}

	@Test(priority = 2)
	public void AIHCAWithLessThan45percentage() throws Exception {

		// logger = report.createTest("Verifying AIHCA Department Not Apply Less Than 45
		// Percenatge");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 65, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 65, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked AIHCA Department Not Apply Less Than 45 Percenatge Scenario");
		}
	}
}
