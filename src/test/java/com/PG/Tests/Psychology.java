...
public class Psychology extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void PsychologyWithAtleast45percentage() throws Exception {

		// logger = report.createTest("Verifying Psychology PG Department Apply Minimum
		// 45 Percentage");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Psychology");
			Thread.sleep(500);
			From.PGSubjectofAdmission("Psychology");
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 207, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationCancel().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 207, "Fail");
			}
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked Psychology Department Apply Minimum 45 percentage Scenario");
		}

	}

	@Test(priority = 2)
	public void PsychologyWithNotLessThan45percentage() throws Exception {

		// logger = report.createTest("Verifying Psychology Department Not Apply Less
		// Than 45 Percenatge");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting Psychology");
			Thread.sleep(400);
			From.PGSubjectofAdmission("Psychology");
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 208, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 208, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked Psychology Department Not Apply Less Than 45 Percenatge Scenario");
		}
