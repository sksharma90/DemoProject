...
public class AppliedGeography extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void AppliedGeographyWithAtleast45Percentage() throws Exception {

		// logger = report.createTest("Applied Geography with Atleast 45 Percentage");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting Applied Geography");
			Thread.sleep(400);
			From.PGSubjectofAdmission("Applied Geography");
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 78, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationCancel().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 78, "Fail");
			}
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked Applied Geography PG Department with Atleast 45 Percentage");
		}

	}

	@Test(priority = 2)
	public void AppliedGeographyWithLessThan45Percentage() throws Exception {

		// logger = report.createTest("Verify Applied Geography with Less Than 45
		// Percentage");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 79, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 79, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked Applied Geography PG Department  with Less Than 45 Percentage");
		}
