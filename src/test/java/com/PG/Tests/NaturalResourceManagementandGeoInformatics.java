...
public class NaturalResourceManagementandGeoInformatics extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void NaturalResourceManagementGeoInformaticsWithAtleast45Percentage() throws Exception {

		// logger = report.createTest("Verify Natural Resource Management GeoInformatics
		// Apply Atleast 45 Percentage");
			...
			if (defaultItem.equals("Science")) {
				...
				for (int j = 0; j < arrData1.length; j++) {
					...
					ExtentFactory.getInstance().getExtent()
							.info("Selecting Natural Resource Management & Geo Informatics");
					Thread.sleep(400);
					From.PGSubjectofAdmission("Natural Resource Management & Geo Informatics");
...
//					From.EducationalQualificationCancel().click();
					...
					{
						constants.fileone.setCellData(sheetName, "Status", 193, "Pass");
						Thread.sleep(300);
						From.EducationalQualificationCancel().click();
					}

					else {
						constants.fileone.setCellData(sheetName, "Status", 193, "Fail");
					}
					...
					ExtentFactory.getInstance().getExtent().info(
							"Successfully Checked NaturalResourceManagementGeoInformatics PG Department Atleast 45 Percentage scenario");
				}
			}
		}
	}
	...
	@Test(priority = 2)
	public void NaturalResourceManagementGeoInformaticsWithLessThan45Percentage() throws Exception {

		// logger = report.createTest("Verify Natural Resource Management GeoInformatics
		// with Less Than 45 Percentage");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			if (defaultItem.equals("Science")) {
				...
				for (int j = 0; j < arrData1.length; j++) {
					...
					{
						constants.fileone.setCellData(sheetName, "Status", 194, "Pass");
						Thread.sleep(300);
						From.EducationalQualificationOk().click();
					}

					else {
						constants.fileone.setCellData(sheetName, "Status", 194, "Fail");
					}
					SoftAssert softAssert = new SoftAssert();
					...
					ExtentFactory.getInstance().getExtent().info(
							"Successfully Checked NaturalResourceManagementGeoInformatics PG Department with Less Than 45 Percentage scenario");
				}
			}
		}
	}
