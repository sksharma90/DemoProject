...
public class MBARuralManagement extends BaseClass implements PGCAFData{
	...
	@Test(priority = 1)
	public void RuralDevelopmentWithAtleast50Percentage() throws Exception {

		//logger = report.createTest("Verify Rural Development Apply Atleast 50 Percentage");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("MBA (Rural Management)");
			Thread.sleep(1000);
			From.PGSubjectofAdmission("MBA (Rural Management)");
			...
			ExtentFactory.getInstance().getExtent().info("Successfully Checked RuralDevelopment PG Department Atleast 50 Percentage scenario");
		}

	}

	@Test(priority = 2)
	public void RuralDevelopmentWithLessThan50Percentage() throws Exception {

		//logger = report.createTest("Verify Rural Development Not Apply Less Than 50 Percentage");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting RuralDevlopment");
			Thread.sleep(400);
			From.PGSubjectofAdmission("MBA (Rural Management)");
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 176, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 176, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
			...
			ExtentFactory.getInstance().getExtent().info("Successfully Checked RuralDevelopment PG Department Not Apply Less Than 50 Percentage scenario");
		}
