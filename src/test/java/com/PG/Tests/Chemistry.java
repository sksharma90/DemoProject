...
public class Chemistry extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void ChemistryWithAtleast45percentage() throws Exception {

		// logger = report.createTest("Verifying for Chemistry PG department with
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting Chemistry");

			Thread.sleep(400);
			From.PGSubjectofAdmission("Chemistry");
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 105, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationCancel().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 105, "Fail");
			}
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully checked applying for Chemistry department with Minimum 45 percentage Scenario");
		}

	}

	@Test(priority = 2)
	public void ChemistryWithLessThan45percentage() throws Exception {

		// logger = report.createTest("Verifying for Chemistry PG department with Less
		// Than 45 Percenatge");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 106, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 106, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
			...
			ExtentFactory.getInstance().getExtent().info(
					"Successfully checked applying for Chemistry department with Less Than 45 Percenatge Scenario");
		}
