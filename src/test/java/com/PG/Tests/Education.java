...
public class Education extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void EducationWithAtleast45percentage() throws Exception {

		// logger = report.createTest("Verifying for Education PG department with
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Education");

			Thread.sleep(400);
			From.PGSubjectofAdmission("Education");
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 115, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationCancel().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 115, "Fail");
			}
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully checked applying for Education department with Minimum 45 percentage Scenario");
		}

	}

	@Test(priority = 2)
	public void EducationWithLessThan45percentage() throws Exception {

		// logger = report.createTest("Verifying for Education PG department with Less
		// Than 45 Percenatge");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 116, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 116, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
			...
			ExtentFactory.getInstance().getExtent().info(
					"Successfully checked applying for Education department with Less Than 45 Percenatge Scenario");
		}
