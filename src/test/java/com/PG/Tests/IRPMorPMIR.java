...
public class IRPMorPMIR extends BaseClass implements PGCAFData{
	...
	@Test(priority = 1)
	public void PMIRWithAtleast45percentage() throws Exception {

		//logger = report.createTest("Verifying PMIR PG Department Apply Minimum 45 Percentage");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting Industrial Relations & Personnel Management(IRPM/PMIR)");
			Thread.sleep(400);
			From.PGSubjectofAdmission("Industrial Relations & Personnel Management(IRPM/PMIR)");
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 143, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationCancel().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 143, "Fail");
			}
			...
			ExtentFactory.getInstance().getExtent().info("Successfully Checked PMIR Department Apply Minimum 45 percentage Scenario");
		}

	}

	@Test(priority = 2)
	public void PMIRWithLessThan45percentage() throws Exception {

		//logger = report.createTest("Verifying PMIR Department Not Apply Less Than 45 Percenatge");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked PMIR Department Not Apply Less Than 45 Percenatge Scenario");
		}
