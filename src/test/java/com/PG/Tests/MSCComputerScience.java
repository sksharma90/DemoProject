...
public class MSCComputerScience extends BaseClass implements PGCAFData{
	...
	@Test(priority = 1)
	public void MSCComputerScienceWithAtleast45Percentage() throws Exception {

		//logger = report.createTest("Verify MSC Computer Science Apply Atleast 45 Percentage");
		...
		String[] arrData = { "BTech", "BE", "BCA", "B.Sc. (ITM)", "B.Sc. (CS)", "B.Sc. (IT)", "B.Sc. (IST)" };
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting MSCComputerScience");
			Thread.sleep(400);
...
//			From.CollegeUniversity();
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 187, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationCancel().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 187, "Fail");
			}
			...
			ExtentFactory.getInstance().getExtent().info("Successfully Checked MSCComputerScience Pg Department Atleast 45 Percentage scenario");
		}

	}

	@Test(priority = 2)
	public void MSCComputerScienceWithLessThan45Percentage() throws Exception {

		//logger = report.createTest("Verify MSC Computer Science NotApply Less Than 45 Percentage");
...
//			From.CollegeUniversity();
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 188, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 188, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
			...
			ExtentFactory.getInstance().getExtent().info("Successfully Checked MSCComputerScience PG Department Not Apply Less Than 45 Percentage scenario");
		}
