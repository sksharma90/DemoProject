...
public class Urdu extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void UrduDepartmentWithAtleast45Percentage() throws Exception {

		// logger = report.createTest("Verify Urdu Department Apply Atleast 45
		// Percentage");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting UrduDepartment");
			Thread.sleep(400);
			From.PGSubjectofAdmission("Urdu");
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 224, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationCancel().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 224, "Fail");
			}
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked Urdu PG Department Atleast 45 Percentage scenario");
		}

	}

	@Test(priority = 2)
	public void UrduDepartmentWithLessThan45Percentage() throws Exception {

		// logger = report.createTest("Verify Urdu Department Not Apply Less Than 45
		// Percentage");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 225, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 225, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked Urdu Pg Department Not Apply Less Than 45 Percentage scenario");
		}
