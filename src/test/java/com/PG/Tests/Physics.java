...
public class Physics extends BaseClass implements PGCAFData{
	...
	@Test(priority = 1)
	public void PhysicsWithAtleast45percentage() throws Exception {

		//logger = report.createTest("Verifying Physics PG Department Apply Minimum 45 Percentage");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting Physics");
			Thread.sleep(400);
			From.PGSubjectofAdmission("Physics");
			...
			ExtentFactory.getInstance().getExtent().info("Successfully Checked Physics Department Apply Minimum 45 percentage Scenario");
		}

	}

	@Test(priority = 2)
	public void PhysicsWithLessThan45percentage() throws Exception {

		//logger = report.createTest("Verifying Physics Department Not Apply Less Than 45 Percentage");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked Physics Department Not Apply Less Than 45 Percenatge Scenario");
		}
