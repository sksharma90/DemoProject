...
public class Linguistics extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void LinguisticsWithAtleast45percentage() throws Exception {

		// logger = report.createTest("Verifying Linguistics PG Department Apply Minimum
		// 45 Percentage");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting Linguistics");
			Thread.sleep(400);
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 151, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationCancel().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 151, "Fail");
			}
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked Linguistics Department Apply Minimum 45 percentage Scenario");
		}

	}

	@Test(priority = 2)
	public void LinguisticsWithLessThan45percentage() throws Exception {

		// logger = report.createTest("Verifying Linguistics Department Not Apply Less
		// Than 45 Percenatge");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 152, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 152, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked Linguistics Department Not Apply Less Than 45 Percenatge Scenario");
		}
