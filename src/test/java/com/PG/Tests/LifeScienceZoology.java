...
public class LifeScienceZoology extends BaseClass implements PGCAFData {
	...
	public void LifeScienceWithAtleast45percentage() throws Exception {
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			if (defaultItem.equals("Science")) {
				Thread.sleep(300);
				String[] arrData1 = { "Zoology", "Animal Science", "Biotechnology" };
				for (int j = 0; j < arrData1.length; j++) {
					...
					ExtentFactory.getInstance().getExtent().info("Life Science(Zoology)");
					Thread.sleep(400);
					From.PGSubjectofAdmission("Life Science(Zoology)");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			if (defaultItem.equals("Science")) {
				...
				for (int j = 0; j < arrData1.length; j++) {
					...
					Thread.sleep(300);
					From.Elective1Credit1("24");
					ExtentFactory.getInstance().getExtent().info("Selecting Life Science(Zoology)");
					Thread.sleep(400);
					From.PGSubjectofAdmission("Life Science(Zoology)");
