...
public class AppliedChemistry extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void AppliedChemistryWithAtleast45percentage() throws Exception {

		// logger = report.createTest("Verifying Applied Chemistry PG Department with
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting Applied Chemistry");
			Thread.sleep(400);
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked Applied Chemistry Department Apply Minimum 45 percentage Scenario");
		}

	}

	@Test(priority = 2)
	public void AppliedChemistryWithLessThan45percentage() throws Exception {

		// logger = report.createTest("Verifying Applied Chemistry Department with Less
		// Than 45 Percenatge");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info(
					"Successfully Checked Applied Chemistry Department Not Apply Less Than 45 Percenatge Scenario");
		}
	}
}
