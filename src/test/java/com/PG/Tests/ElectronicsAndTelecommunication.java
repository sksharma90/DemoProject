...
public class ElectronicsAndTelecommunication extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void ElectronicsAndTelecomunicationWithAtleast50Percentage() throws Exception {

		// logger = report.createTest("Verify Electronics And Telecomunication with
		...
		String[] arrData = { "Science", "BTech", "B.Sc. (IST)", "B.Sc. (IT)", "B.Sc. (ITM)" };
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			if (defaultItem.equals("B.Sc. (ITM)")) {
			...
			else if (defaultItem.equals("BTech")) {

				String[] arrData1 = { "Information Technology", "Computer Science Engg.",
						"Electrical and Electronics Engg.", "Electronics and Communication Engg.",
				...
				for (int j = 0; j < arrData1.length; j++) {
					...
					ExtentFactory.getInstance().getExtent().info("Selecting Electronics and Telecommunication (ETC)");
					Thread.sleep(400);
					From.PGSubjectofAdmission("Electronics and Telecommunication (ETC)");
				...
				Thread.sleep(300);
				String[] arrData1 = { "Physics", "Electronics", "Computer Science", "Mathematics" };
				for (int j = 0; j < arrData1.length; j++) {
					Thread.sleep(500);
					...
					Thread.sleep(300);
					From.GraduationElective1("Chemistry");
			...
			if (defaultItem.equals("B.Sc. (ITM)") || defaultItem.equals("B.Sc. (IT)")
					|| defaultItem.equals("B.Sc. (IST)")) {
				...
				{
					constants.fileone.setCellData(sheetName, "Status", 117, "Pass");
					Thread.sleep(300);
					From.EducationalQualificationCancel().click();
				}

				else {
					constants.fileone.setCellData(sheetName, "Status", 117, "Fail");
				}
				...
				ExtentFactory.getInstance().getExtent().info(
						"Successfully Checked ElectronicsAndTelecomunication Pg Department Atleast 50 Percentage");
			}
		}
	}

	@Test(priority = 2)
	public void ElectronicsAndTelecomunicationWithLessThan50Percentage() throws Exception {
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			if (defaultItem.equals("B.Sc. (ITM)") || defaultItem.equals("B.Sc. (IT)")
				...
				ExtentFactory.getInstance().getExtent().info(
						"Successfully Checked ElectronicsAndTelecomunication PG Department Not Apply Less Than 50 Percentage");
			}
		}
	}
