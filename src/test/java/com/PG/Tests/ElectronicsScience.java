...
public class ElectronicsScience extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void ElectronicsScienceWithAtleast45Percentage() throws Exception {
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			if (defaultItem.equals("B.Sc. (ITM)")) {
				...
				for (int j = 0; j < arrData1.length; j++) {
					...
					ExtentFactory.getInstance().getExtent().info("Selecting Electronics Science");
					Thread.sleep(400);
...
//				From.EducationalQualificationCancel().click();
	...
	@Test(priority = 2)
	public void ElectronicsScienceWithLessThan45Percentage() throws Exception {
