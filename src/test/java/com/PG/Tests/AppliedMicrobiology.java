...
public class AppliedMicrobiology extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void AppliedMicrobiologyWithAtleast45percentage() throws Exception {

		// logger = report.createTest("Verifying Applied Microbiology PG Department with
		// Minimum 45 Percentage");
		...
		String[] arrData = { "Botany", "Microbiology", "Zoology" };
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting Applied Microbiology");
			Thread.sleep(400);
			From.PGSubjectofAdmission("Applied Microbiology");
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 82, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationCancel().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 82, "Fail");
			}
			...
			ExtentFactory.getInstance().getExtent()
					.info("Sucessfully Checked Applied Microbiology Department Apply Minimum 45 percentage Scenario");
		}

	}

	@Test(priority = 2)
	public void AppliedMicrobiologyWithLessThan45percentage() throws Exception {

		// logger = report.createTest("Verifying Applied Microbiology Department with
		// Less Than 45 Percentage");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 83, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 83, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked Applied Microbiology Department with Less Than 45 Percentage Scenario");
		}
	}
}
