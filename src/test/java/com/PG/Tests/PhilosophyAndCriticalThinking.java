...
public class PhilosophyAndCriticalThinking extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void PhilosphyAndCriticalThinkingWithAtleast45Percentage() throws Exception {

		// logger = report.createTest("Verify Philosphy And Critical Thinking Apply
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting PhilosphyAndCriticalThinking");
			Thread.sleep(600);
			From.PGSubjectofAdmission("Philosophy and Critical Thinking");

			Thread.sleep(400);

			// Thread.sleep(500);
			// From.CollegeorUniversityPreference("Udaya Nath College of Science &
			// Technology,Adaspur");
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 199, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationCancel().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 199, "Fail");
			}
			...
			ExtentFactory.getInstance().getExtent().info(
					"Successfully Checked PhilosphyAndCriticalThinking Pg Department Atleast 45 Percentage scenario");
		}

	}

	@Test(priority = 2)
	public void PhilosphyAndCriticalThinkingWithLessThan45Percentage() throws Exception {

		// logger = report.createTest("Verify Philosphy And Critical Thinking Not Apply
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info(
					"Successfully Checked PhilosphyAndCriticalThinking PG Department Not Apply Less Than 45 Percentage scenario");
		}
