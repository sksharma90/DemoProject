...
public class Hindi extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void HindiWithAtleast45percentage() throws Exception {

		// logger = report.createTest("Verifying Hindi PG Department Apply Minimum 45
		// Percentage");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Hindi");
			Thread.sleep(400);
			From.PGSubjectofAdmission("Hindi");
			...
			Thread.sleep(400);
			From.FirstPreference("Fakir Mohan (Autonomous) College, Balasore");
			Thread.sleep(400);
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 137, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationCancel().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 137, "Fail");
			}
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked Hindi Department Apply Minimum 45 percentage Scenario");
		}

	}

	@Test(priority = 2)
	public void HindiWithLessThan45percentage() throws Exception {

		// logger = report.createTest("Verifying Hindi Department Not Apply Less Than 45
		// Percenatge");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting Hindi");
			Thread.sleep(400);
			From.PGSubjectofAdmission("Hindi");
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 138, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 138, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked Hindi Department with Less Than 45 Percenatge Scenario");
		}
