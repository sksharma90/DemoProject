...
public class BioTechnology extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void BiotechnologyWithAtleast50Percentage() throws Exception {

		// logger = report.createTest("Verify Biotechnology with Atleast 50
		// Percentage");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			if (defaultItem.equals("BTech")) {
				...
				String[] arrData1 = { "Physics", "Chemistry", "Botany", "Biotechnology", "Zoology", "Mathematics",
						"Microbiology", "Biochemistry", "Bio informatic", "Life Science" };
				for (int j = 0; j < arrData1.length; j++) {
					...
					ExtentFactory.getInstance().getExtent().info("Selecting Biotechnology");
					Thread.sleep(400);
					From.PGSubjectofAdmission("Biotechnology");
			...
			if (defaultItem.equals("BTech") || defaultItem.equals("BHMS") || defaultItem.equals("BE")
					|| defaultItem.equals("MBBS") || defaultItem.equals("B Pharma")) {
				...
				ExtentFactory.getInstance().getExtent()
						.info("Successfully Checked Biotechnology PG Department Atleast 50 Percentage");
			}
		}
	}

	@Test(priority = 2)
	public void BiotechnologyWithLessThan50Percentage() throws Exception {

		// logger = report.createTest("Verify Biotechnology with Less Than 50
		// Percentage");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			if (defaultItem.equals("BTech") || defaultItem.equals("BHMS") || defaultItem.equals("BE")
				...
				ExtentFactory.getInstance().getExtent()
						.info("Successfully Checked Biotechnology PG Department with Less Than 50 Percentage");
			}
		}
	}
