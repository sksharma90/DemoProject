...
public class NonAcceptanceOfDuplicateExamCenter extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void VerifyNonAcceptanceOfDuplicateExamCenter() throws Exception {

		// logger = report.createTest("Verify the Non-Acceptance Of Duplicate Exam
		// Center");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting Duplicate Exam Center");
			Thread.sleep(400);
			From.FirstPreference("Berhampur University, Bhanja Vihar, Berhampur");
			Thread.sleep(400);

			From.SecondPreference("Berhampur University, Bhanja Vihar, Berhampur");
			Thread.sleep(400);
			...
			if (actualPopup.contains("Same Centre can not selected more than once !"))

			{
				constants.fileone.setCellData(sheetName, "Status", 240, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 240, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
			softAssert.assertEquals(actualPopup, "Same Centre can not selected more than once !", "Test Failed");

			softAssert.assertAll();

			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked Non-Acceptance of Duplicate Exam Center Scenario");
		}
