...
public class MarineScienceMarineBiology extends BaseClass implements PGCAFData{
	...
	@Test(priority = 1)
	public void MarineScienceBiologyWithAtleast45percentage() throws Exception {

		//logger = report.createTest("Verifying Marine Science Marine Biology PG Department Apply Minimum 45 Percentage");
		...
		String[] arrData = { "Biotechnology", "Botany", "Zoology", "Microbiology", "Fishery Science", "Life Science" };
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting Marine Science - Marine Biology");
			Thread.sleep(400);
			From.PGSubjectofAdmission("Marine Science - Marine Biology");
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 159, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationCancel().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 159, "Fail");
			}
			...
			ExtentFactory.getInstance().getExtent().info(
					"Successfully Checked Marine Science Marine Biology Department Apply Minimum 45 percentage Scenario");
		}

	}

	@Test(priority = 2)
	public void MarineScienceBiologyWithLessThan45percentage() throws Exception {

		//logger = report.createTest("Verifying MarineScienceMarineBiology Department with Less Than 45 Percenatge");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info(
					"Successfully Checked MarineScienceMarineBiology Department with Less Than 45 Percenatge Scenario");
		}
	}
