...
public class AnthropologyAndTribalStudiesArts extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void AnthropologyAndTribalStudiesWithAtleast45Percentage() throws Exception {

		// logger =
		// report.createTest("VerifyAnthropologyAndTribalStudiesApplyAtleast45Percentage");
		...
		for (int j = 0; j < arrData.length; j++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting Anthropology & Tribal Studies (ARTS)");
			Thread.sleep(400);
			From.PGSubjectofAdmission("Anthropology & Tribal Studies (ARTS)");
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 70, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationCancel().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 70, "Fail");
			}
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked AnthropologyAndTribalStudies PG Department Atleast 45 Percentage");
		}
	}

	@Test(priority = 2)
	public void AnthropologyAndTribalStudiesWithLessThan45Percentage() throws Exception {

		// logger = report.createTest("Anthropology And Tribal Studies with Less Than 45
		// Percentage");
		...
		for (int j = 0; j < arrData.length; j++) {
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 71, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 71, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
			...
			ExtentFactory.getInstance().getExtent().info(
					"Successfully Checked AnthropologyAndTribalStudies PG Department with Less Than 45 Percentage");
		}
	}
}