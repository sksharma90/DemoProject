...
public class Odia extends BaseClass implements PGCAFData {
	...
	@Test(priority = 0)
	public void OdiaWithAtleast45percentage() throws Exception {

		// logger = report.createTest("Verifying Odia PG Department Apply Minimum 45
		// Percentage");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting Odia");
			Thread.sleep(400);
			From.PGSubjectofAdmission("Odia");
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 195, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationCancel().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 195, "Fail");
			}
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked Odia Department Apply Minimum 45 percentage Scenario");
		}

	}

	@Test(priority = 2)
	public void OdiaWithLessThan45percentage() throws Exception {

		// logger = report.createTest("Verifying Odia Department Not Apply Less Than 45
		// Percenatge");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 196, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 196, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked Odia Department with Less Than 45 Percenatge Scenario");
		}
