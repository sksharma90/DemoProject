...
public class JournalismAndMassCommunication extends BaseClass implements PGCAFData{
	...
	@Test(priority = 1)
	public void JAMCWithAtleast45Percentage() throws Exception {
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting Journalism and Mass Communication");
			Thread.sleep(400);
			From.PGSubjectofAdmission("Journalism and Mass Communication");
			...
			ExtentFactory.getInstance().getExtent().info("Successfully Checked Journalism and Mass Communication Atleast 45 Percentage scenario");
		}

	}

	@Test(priority = 2)
	public void JAMCWithLessThan45Percentage() throws Exception {
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 146, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 146, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
			...
			ExtentFactory.getInstance().getExtent().info("Successfully Checked Journalism and Mass Communication with Less Than 45 Percentage scenario");
		}
