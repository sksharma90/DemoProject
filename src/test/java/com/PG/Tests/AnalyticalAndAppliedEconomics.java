...
public class AnalyticalAndAppliedEconomics extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void AnalyticalAndAppliedEconomicsWithAtleast45Percentage() throws Exception {
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting Analytical & Applied Economics");
			Thread.sleep(600);
			From.PGSubjectofAdmission("Analytical & Applied Economics");
...
//			Thread.sleep(400);
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 66, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationCancel().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 66, "Fail");
			}
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked Analytical And Applied Economics with Atleast 45 Percentage");
		}
	}

	@Test(priority = 2)
	public void AnalyticalAndAppliedEconomicsWithLessThan45Percentage() throws Exception {
...
//			Thread.sleep(400);
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 67, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 67, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
			...
			ExtentFactory.getInstance().getExtent()
					.info("Successfully Checked Analytical And Applied Economics  with Less Than 45 Percentage");
		}
