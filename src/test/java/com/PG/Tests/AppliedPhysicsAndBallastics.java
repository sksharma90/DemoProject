...
public class AppliedPhysicsAndBallastics extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void AppliedPhysicsAndBallasticsWithAtleast45percentage() throws Exception {

		// logger = report.createTest("Verifying Applied Physics And Ballastics PG
		// Department with Minimum 45 Percentage");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting Applied Physics & Ballastics");
			Thread.sleep(400);
			From.PGSubjectofAdmission("Applied Physics & Ballastics");
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 86, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationCancel().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 86, "Fail");
			}
			...
			ExtentFactory.getInstance().getExtent().info(
					"Successfully Checked Applied Physics And Ballastics Department with Minimum 45 percentage Scenario");
		}

	}

	@Test(priority = 2)
	public void AppliedPhysicsAndBallasticsWithLessThan45percentage() throws Exception {

		// logger = report.createTest("Verifying Applied Physics And Ballastics
		// Department with Less Than 45 Percenatge");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 87, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 87, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
			...
			ExtentFactory.getInstance().getExtent().info(
					"Successfully Checked Applied Physics And Ballastics Department with Less Than 45 Percenatge Scenario");
		}
	}
}
