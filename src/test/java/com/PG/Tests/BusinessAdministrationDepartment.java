...
public class BusinessAdministrationDepartment extends BaseClass implements PGCAFData{
	...
	@Test(priority = 1)
	public void BusinessAdministrationWithAtleast50percentage() throws Exception {

		//logger = report.createTest("Verifying for Business Administration PG department with atleast 50 percentage");
		...
		String[] arrData = { "BTech", "BE", "BBA", "BCA" };
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			//logger = report.createTest("Selecting Business Administration");

			Thread.sleep(400);
			From.PGSubjectofAdmission("MBA (Business Admistration)");
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 76, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationCancel().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 76, "Fail");
			}
			...
			ExtentFactory.getInstance().getExtent().info(
					"Successfully checked applying for Business Administration department with atleast 50 percentage scenario");
		}

	}

	@Test(priority = 2)
	public void BusinessAdministrationWithLessThan50percentage() throws Exception {

		//logger = report.createTest("Verifying for Business Administration PG department with Less Than 50 Percenatge");
		...
		for (int i = 0; i < arrData.length; i++) {
			...
			ExtentFactory.getInstance().getExtent().info("Selecting Business Administration");
			Thread.sleep(400);
			From.PGSubjectofAdmission("MBA (Business Admistration)");
			...
			{
				constants.fileone.setCellData(sheetName, "Status", 77, "Pass");
				Thread.sleep(300);
				From.EducationalQualificationOk().click();
			}

			else {
				constants.fileone.setCellData(sheetName, "Status", 77, "Fail");
			}
			SoftAssert softAssert = new SoftAssert();
			...
			ExtentFactory.getInstance().getExtent().info(
					"Successfully checked applying for Business Administration department with Less Than 50 Percenatge Scenario");
		}
