...
public class AgeValidation extends BaseClass implements PGCAFData {
	...
	@Test(priority = 1)
	public void SubmittingCAFWithValidAge() throws Exception {

		//logger = report.createTest("Verifying submission of CAF by Applicant having more than 13 years of age");
		...
		ExtentFactory.getInstance().getExtent().info("Providing valid age in Date of Birth");
		...
		ExtentFactory.getInstance().getExtent().info("Successfully checked Valid age scenario");
	}

	@Test(priority = 2)
	public void SubmittingCAFWithInvalidAge() throws Exception {

		//logger = report.createTest("Verifying submission of CAF by Applicant having less than 13 years of age");
		...
		ExtentFactory.getInstance().getExtent().info("Providing Invalid age in Date of Birth");

		From.InValidBirthDate();
		Thread.sleep(300);
		...
		if (actualPopup.contains("Difference between Year of birth and 10th Year Of Passing Should be minimum 13 !"))
		...
		softAssert.assertEquals(actualPopup,
				"Difference between Year of birth and 10th Year Of Passing Should be minimum 13 !", "Test Failed");
		
		softAssert.assertAll();

		ExtentFactory.getInstance().getExtent().info("Successfully checked Invalid age scenario");
	}
